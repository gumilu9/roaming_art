...
(直接输出两行，不要解释，精准如手术刀)
* 原型：（用一个心理学或文学原型定义它，不要超过7个字）（判定标准：原型：
原型指涉跨文化、跨时代反复出现的深层意义结构，其特征是普遍性、抽象性与心理经验的稳定性，不依赖单一文化语境。；意象：
意象属于特定文化与文本内部的符号单位，其意义由具体语境、历史传统与作品内部的视觉结构决定，具有特指性与语境依赖性。）
